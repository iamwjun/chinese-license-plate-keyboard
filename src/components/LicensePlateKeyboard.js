...
export default class LicensePlateKeyboard extends Component {
    ...
    handleClick = (e, m) => {
        if (this.state.position + 1 <= this.state.licenseLen) {
            const forms = this.state.license;
    ...
    handleMap = (source, type) => {
        return <ul className={type ? 'province' : 'alphanumeric'}>
        {
          source.map((item) => (
            item.map((m) => (
              <li key={m.toString()} onClick={
                (e) => this.handleClick(e, m.toString())
              }>
                <span>{m}</span>
              </li>
            ))
      ...
      return <ul className="enterform">
        {
          license.map((n, index) => (
            <li key={index} onClick={
              (e) => this.handleLicenseForm(e, index)}><span className={
                `${index == this.state.position ? 'blue' : ''
                } 
                    ${this.state.notNew && n == '+新' ? 'new' : ''}`
              }>{n}</span></li>
          ))
        }
      </ul>;
